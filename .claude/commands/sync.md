...
```
@jogi/reports dirty (N files, M LOC). Commit + push? (y / n / w)
...
INSTALLED_SHA=$(grep -oE 'jogi-reports#[a-f0-9]{40}' ../jogi/package-lock.json | head -1 | grep -oE '[a-f0-9]{40}')
```
...
```bash
cd ../jogi
npm install @jogi/reports@github:luvidal/jogi-reports#$LOCAL_SHA
npx tsc --noEmit
```
